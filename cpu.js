class PlayerCPU{
    ...
    update(){
        ...
        if(this.state !== this.STATE.JUMP){
            ...
            if(!this.vision()) this.velocity.x = -BLIND_WALK;
            else if(this.vision()) this.velocity.x = -WALK;
            else if(this.dist === 0) this.vision.x = 0;
    ...
    vision() {
        ...
        if(this.dist == 0) console.log("ZERO ZERO ZERO");
        return (this.dist < this.radius + this.other.radius);
    };
};
